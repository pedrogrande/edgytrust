...
- [ ] Create: `database/run-migrations.ts` (migration runner script)
- [ ] Run: `npm run migrate`
- [ ] Verify: `psql $DATABASE_URL -c "\dt"` (should show 17 tables)
  - [ ] `events` (immutable)
  - [ ] `verification_reports` (immutable)
  - [ ] `agent_artifacts` (immutable)
  - [ ] `task_execution_notes` (immutable)
  - [ ] `agent_specifications` (reference)
  - [ ] `reference_documentation` (reference)
  - [ ] `ontology_definitions` (reference)
  - [ ] `raci_matrices` (reference)
  - [ ] `artifact_schemas` (reference)
  - [ ] `task_contracts` (mutable)
  - [ ] `agent_profiles` (mutable)
